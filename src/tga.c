...
AFB_ERROR tga_load_file(IMAGE *img, char *path)
{
	...
	if (imageType == TGA_IMG_TRUECOLOR) {
		for (i = 0; i < image_size; i += 3) {
			fread(&img->image_data[i + 2], sizeof(img->image_data[0]), 1,
			      f_tga);
			fread(&img->image_data[i + 1], sizeof(img->image_data[0]), 1,
			      f_tga);
			fread(&img->image_data[i + 0], sizeof(img->image_data[0]), 1,
			      f_tga);
		}
	} else {
