...
static AFB_ERROR nearest_neighbor_rgb(IMAGE *img, uint32_t w, uint32_t h)
{
	...
	for (int y=0; y < h; y++) {
		for (int x=0; x < w; x++) {
			x2 = ((x * x_ratio) >> 16);
			y2 = ((y * y_ratio) >> 16);
			new_image_data[((y * w) + x) * 3] =     img->image_data[((y2 * img->width) + x2) * 3];
			new_image_data[((y * w) + x) * 3 + 1] = img->image_data[((y2 * img->width) + x2) * 3 + 1];
			new_image_data[((y * w) + x) * 3 + 2] = img->image_data[((y2 * img->width) + x2) * 3 + 2];
		}
	}
