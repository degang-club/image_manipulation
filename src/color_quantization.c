...
{
	return ((*(uint32_t *) a & 0xff000000) >
	        (*(uint32_t *) b & 0xff000000)) - ((*(uint32_t *) a & 0xff000000) <
	                                           (*(uint32_t *) b & 0xff000000));
}

static int cmp_g(const void *a, const void *b)
{
	return ((*(uint32_t *) a & 0x00ff0000) >
	        (*(uint32_t *) b & 0x00ff0000)) - ((*(uint32_t *) a & 0x00ff0000) <
	                                           (*(uint32_t *) b & 0x00ff0000));
}

static int cmp_b(const void *a, const void *b)
{
	return ((*(uint32_t *) a & 0x0000ff00) >
	        (*(uint32_t *) b & 0x0000ff00)) - ((*(uint32_t *) a & 0x0000ff00) <
...
AFB_PALETTE afb_unique_colors(uint8_t *img_data, unsigned int img_size)
{
	...
	for (int i = 0; i < img_size; i++) {
		pal_w.colors[i] =
		    afb_to_rgba(img_data[i * 3], img_data[i * 3 + 1],
		                img_data[i * 3 + 2], 0);
		pal_w.size++;
	}
...
AFB_PALETTE quantize_median_cut(AFB_IMAGE img, unsigned int palette_size)
{
	...
	for (int i = 0; i < palette_size; i++) {
		...
		*bucket_list[i] = (BUCKET) {
		    .palette.size = 0, .palette.colors = 0, .ranges = { 0, 0, 0 },
			.largest_channel = 0};
	}
	...
	for (int i = 1; i < palette_size; i++) {
		...
		for (size_t j = 0; j < total_buckets; j++) {
			largest_channel =
			    bucket_list[j]->ranges[bucket_list[j]->largest_channel];
		...
		bucket_list[total_buckets]->palette.colors =
		    &bucket_list[bucket_to_split]->palette.colors[split_bucket_size_1];
